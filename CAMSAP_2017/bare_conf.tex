...
Although there some works to  which the \gls{HB} and channel estimation are
considered such as \cite{Venugopal:2017,Heath:2016,Alkhateeb:2014}, but the
frequency selectivity is not consider.
...
selective channels that relies on the sparse \gls{PARAFAC} analysis. 
proposed  
...
condition is achieved, the estimation is improved the more transmit beams are
used at the cost of more overhead.


Fig \ref{fig:rx_beams} shows another to improve the channel estimation. Instead
of increasing the number of transmit beams, it is possible to increase the
number of receive beams \gls{nRxBeam} that measures that channel within the same
frame. This will require more \gls{RF} chains to be used at the same time, so there
is a trade-off among hardware complexity, system overhead, and the accuracy to
achieve so that system can offer good spectral efficiency (reduced overhead) but
using  transceivers with reduced number of combiners.
Nevertheless the increasing of the number of transmit beams can afford to reduce
the pilot sequence length, it cannot be $\gls{pLength}<6$ as it violates the
first corollary, and the uniqueness of the \gls{PARAFAC} model does not hold. 
...
channels using \gls{HB} to transmit the pilot sequences. The proposed method
exploits jointly the intrinsic sparsity in the three dimensions \gls{AoA},
\gls{AoD} and delay. Using the sparse \gls{PARAFAC} model, we ensure minimum
...
as it can conveniently control the system overhead  according to the hardware
limitation and spectral efficiency by choosing the number of beams and pilot

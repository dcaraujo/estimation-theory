...
receiver have a hybrid beamforming structure with \gls{nRFTx} and \gls{nRFTx} chains.
...
\end{equation}
where $\gls{pSym}_{\gls{timeIdx}}$ is the \gls{timeIdx}th non-zero of the
training frame of length $\gls{nTime} + \gls{nTaps}-1$
\begin{equation}
  \label{eq:frame}
  \gls{pSymVec} = [0, \ldots, 0,  \gls{pSym}_1, \gls{pSym}_2, \ldots, \gls{pSym}_{\gls{nTime}} ].
\end{equation}
At the receiver, and a $\gls{RF}$  combiner $\bfw_k$ is applied over the
...
\begin{equation}
  ...
  \gls{rxOutput}_{\gls{rxBeamIdx},\gls{txBeamIdx},\gls{timeIdx}} =  \gls{chTen} \times_1 \gls{rxBeamVec}_{\gls{rxBeamIdx}} \times_2 \gls{txBeamVec}_{\gls{txBeamIdx}} \times \gls{pSymVec}_{\gls{timeIdx}},
\end{equation}

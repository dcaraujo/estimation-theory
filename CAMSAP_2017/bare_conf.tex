...
modeling the channel as a third order tensor. We leverage on this link to derive
a solution that jointly estimates the spatio characteristics of the channel and
the delay pattern of the channel. The proposed method relies on the joint
sparsity of the \gls{AoA}, \gls{AoD}, thereby  affording the system to use
reduced  pilot sequences and very few beams. Moreover, we exploit the
fundamental results of Kruskal for sparse \gls{PARAFAC} models that guarantees
uniqueness. The simulations results show that the can achieve very accurate
estimates at the cost of low-overhead by using the proposed method. 
\end{abstract}
...
\cite{Sidiropoulos:2000,Smilde:2004}. %In the next section, we establish  the
